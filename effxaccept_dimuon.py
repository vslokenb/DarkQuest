...
def cal_eff(file_names,bins = [-300,42]):
    ...
    for i in range(len(bins)-1):
        l_bin = bins[i]
        h_bin = bins[i+1]
        rdf_cut = rdf.Filter(f"truthdimuon_z_vtx[0] > {l_bin} && truthdimuon_z_vtx[0] < {h_bin} && isValidCount(hit_detID)")
        N_tot = rdf_cut.Count().GetValue()
        if N_tot == 0:
            eff = 0
        else:
            N_recoed = rdf_cut.Filter("dimuon_matched[0] > 0").Count().GetValue()
            eff = N_recoed/N_tot
        Effs.append(eff)
    return Effs
...
plt.ylim(0,1)
plt.legend()
plt.title('Dimuon efficiency on accepted events (std tracking)')
#plt.yscale('log')


# Save and show the plot
plt.savefig(f'fast_effxaccept_dimuon_{args.type}_{args.nominal}.pdf', format='pdf')
plt.show()
