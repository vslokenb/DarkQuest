 ...
 #   reco = []
  #  length = []
    ...
    for file in sorted_files:
        try:
            f = uproot.open(file)
            hits = f['Events/n_tracks'].array()  # Number of hits per track
...
#                    print("hi", track_pz[i][0])
 #                   print("ho", hits[i])
                    if track_pz[i][0] < 120 and hits[i] > 0:
                        hit_count.append(hits[i])
  #                      print('hit')
                    else:
                        hit_count.append(0)
   #                     print('nah')
 ...
 #           print(bin_index, 'bin index')
  #          print(len(hits_in_bin), 'hits in bin')
   ...
   # print(total_in_bin)
    ...
    plt.plot(z_bins[:-1], efficiency, label=labels[i], marker='o', color=colors[i % len(colors)])
...
plt.legend()
plt.title('J/Psi vtx position vs track efficiency for different st3 position +sagitta calibration(standard track)')

# Save and show the plot
plt.savefig('jpsi_sagitta_calib_track_eff_all_geom_standard_track.pdf', format='pdf')
plt.show()
