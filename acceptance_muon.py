...
directories = [
    '/seaquest/users/vsloken/scratch/mini_muon_samples/pro_m0_std/',
    '/seaquest/users/vsloken/scratch/mini_muon_samples/pro_m50_std/',
    '/seaquest/users/vsloken/scratch/mini_muon_samples/pro_m100_std/',
    '/seaquest/users/vsloken/scratch/mini_muon_samples/pro_m200_std/'
]
...
#    if not hasattr(number_sort, "entry_count"):
#    number_sort.entry_count = 0
    # Stop sorting after max_entries
 #   if number_sort.entry_count >= max_entries:
  #       return None
  ...
  #  length = []
    ...
    for file in sorted_files:
        try:
            f = uproot.open(file)
 #           print('opened')
            hits = f['Events/hit_detID'].array()  # hits per detector id
#            print(hits[0])
            #track_pz = f['Events/track_pz_st3'].array()  # Track momentum
            z_pos = f['Events/truthtrack_z_vtx'].array()  # Vertex Z positions
...
#                    print("hi", track_pz[i][0])
 #                   print("ho", hits[i])
                    ...
                    for j in range(len(hits[i])):
                        if hits[i][j] < 18 and hits[i][j] < 31:
                            accept+=1
                    if accept >= 4:
                        hit_count.append(1)
  #                      print('hit')
...
plt.ylabel('muon acceptance')
plt.ylim(0,1)
plt.legend()
plt.title('Muon acceptance vs track vtx position for different st3 position (standard track)')
#plt.yscale('log')
# Save and show the plot
plt.savefig('acceptance_muon_stand.pdf', format='pdf')
plt.show()
