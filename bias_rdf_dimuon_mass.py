...
def cal_reso(file_names,bins = [-300,42]):
    ...
    for i in range(len(bins)-1):
        ...
        N_signal0 = N_recoed.Define("pz_diff1","pz_diff1(dimuon_mass, truthdimuon_mass)")
...
plt.ylabel('mass bias (mean) [GeV]')
plt.ylim(-1,1)
plt.legend()
plt.title('Dimuon mass bias  (std tracking)')



# Save and show the plot
plt.savefig(f'final_batch/fast_bias_match_dimuon_mass_std_{args.type}_{args.ecal}.pdf', format='pdf')
plt.show()
