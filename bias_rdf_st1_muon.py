...
double pz_diff1(RVec<double> track_pz_st1, RVec<double> truthtrack_pz_st1) {
    double truth_val1 = truthtrack_pz_st1[truthtrack_pz_st1.size() - 1];
    //double truth_val2 = truthtrack_pz_st1[truthtrack_pz_st1.size() - 2];
    RVec<double> result;
    for (int i=0; i<track_pz_st1.size(); i++) {
        result.push_back((truth_val1 - track_pz_st1[i]));
    }
...
int ang_xz(RVec<double> track_pz_st1, RVec<double> truthtrack_pz_st1, RVec<double> track_px_st1){
    double truth_val = truthtrack_pz_st1[truthtrack_pz_st1.size() - 1];
    RVec<double> result;
    for (int i=0; i<track_pz_st1.size(); i++) {
        if (double(track_px_st1[i]) / double(track_pz_st1[i]) != 0 && double(track_px_st1[i]) / double(track_pz_st1[i]) > -0.05 && double(track_px_st1[i]) / double(track_pz_st1[i]) < 0.05)  {
            return 1;
            }
        else {
            return 0;
...
int validPz(RVec<double> track_pz_st1){
    for (int i=0; i<track_pz_st1.size(); i++) {
    if (double(track_pz_st1[i]) < 120){
        return 1;
    }
    else {
        return 0;
...
def cal_reso(file_names,bins = [-300,42]):
    ...
    for i in range(len(bins)-1):
        ...
        N_recoed = rdf_cut.Filter("n_tracks  > 0 && validPz(track_pz_st1)")
            #N_recoed.Define("px_mask","abs(truthtrack_px_st1[truthtrack_z_vtx.size() - 1] - track_px_st1)")
...
plt.ylabel('pz bias muon gun   [GeV]')
plt.ylim(-8,8)
plt.legend()
plt.title('Muon pz bias at st1 (std tracking)')



# Save and show the plot
plt.savefig(f'final_batch/fast_st1_bias_pz_angmask_muon_{args.type}_{args.ecal}.pdf', format='pdf', bbox_inches="tight")
