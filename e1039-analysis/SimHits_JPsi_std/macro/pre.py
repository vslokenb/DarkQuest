...
for subdir in subdirs:
    ...
    # Dimuon mass plot
    hmass = rdf.Histo1D(("dimuon mass","dimuon_mass",80,0,10),"dimuon_mass")
    ...
    hvtx = rdf.Histo1D(("dimuon z vtx","dimuon_z_vtx",160,-600,800),"dimuon_z_vtx")
    hvtxs.append(hvtx)
    ...
    req = 'MATRIX_5 == 1'
    hmass_FPGA5 = rdf.Filter(req).Histo1D(("dimuon_FPGA5",f"dimuon_mass[{req}]",50,0,10),"dimuon_mass")
    mass_FPGA5.append(hmass_FPGA5)
    
    req = 'dimuon_z_vtx >= -600 && dimuon_z_vtx <= 150'
    hmass_near_targetSQ = rdf.Define("mass_near_target_SQ",f"dimuon_mass[{req}]").Histo1D(("dimuon mass with z<150",f"dimuon_mass[{req}]",80,0,10),"mass_near_target_SQ")
...
cmass = ROOT.TCanvas('c','c')
cmass.cd()
hmass=hmasses[0].Clone()
hmass.Reset()
for hist in hmasses:
    hmass.Add(hist.GetPtr())
hmass.Write()
lamass=ROOT.TLatex(0.25,0.91,"#color[4]{#it{SpinQuest Dark Sector Preliminary}}");
lamass.SetNDC()
hmass.GetXaxis().SetTitle("mass (GeV)")
hmass.GetYaxis().SetTitle("Yield / 2.2 #times 10^{15} POT")
hmass.Draw("pe0")
lamass.Draw("Same")
cmass.Print(f'{hmass.GetName()}.png')
cvtx = ROOT.TCanvas('c','c')
cvtx.cd()
hvtx=hvtxs[0].Clone()
hvtx.Reset()
for hist in hvtxs:
    hvtx.Add(hist.GetPtr())
hvtx.Write()
lavtx=ROOT.TLatex(0.25,0.91,"#color[4]{#it{SpinQuest Dark Sector Preliminary}}");
lavtx.SetNDC()
hvtx.GetXaxis().SetTitle("z_vtx (cm)")
hvtx.GetYaxis().SetTitle("Yield / 2.2 #times 10^{15} POT")
hvtx.Draw("pe0")
lavtx.Draw("Same")
cvtx.Print(f'{hvtx.GetName()}.png')
csq = ROOT.TCanvas('c','c')
csq.cd()
hsq=mass_near_target_SQ[0].Clone()
hsq.Reset()
for hist in mass_near_target_SQ:
    hsq.Add(hist.GetPtr())
hsq.Write()
lasq=ROOT.TLatex(0.25,0.91,"#color[4]{#it{SpinQuest Dark Sector Preliminary}}");
lasq.SetNDC()
hsq.GetXaxis().SetTitle("mass (GeV)")
hsq.GetYaxis().SetTitle("Yield / 2.2 #times 10^{15} POT")
hsq.Draw("pe0")
lasq.Draw("Same")
csq.Print(f'{hsq.GetName()}.png')
'''
