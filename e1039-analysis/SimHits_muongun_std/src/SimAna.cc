...
int SimAna::process_event(PHCompositeNode *topNode)
{
    ...
    if (_saveTracks)
    {
        ...
        int n_st3trackletsINDEX = _legacyContainer ? -1 : _recSt3TrackletVector->size();
        for (int itrk = 0; itrk < n_st3trackletsINDEX; ++itrk)
        {
            ...
            SRecTrack *track = dynamic_cast<SRecTrack *>(_recSt3TrackletVector->at(n_st3tracklets));
    ...
    if (_saveVertex)
    {
        ...
        for (int i = 0; i < nRecDimuons; ++i)
        {
            ...
            dimuon_chisq_vx[n_dimuons] = recDimuon->get_chisq();
